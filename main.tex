...
%\begin{multicols}{3} % Use 3 columns
    ...
    $C \c(\o_0 t + \theta) = C\,\c(\theta) \,\c(\o_0 t) - C\,\s(\o_0)\,\s(\o_0 t)$\\        % highlight minus in cosine
...
\subsubsection{FT Table}
    ...
    $e^{-at} \, u(t) \lra \frac{1}{a+j\omega}$\\         % a > 0
    ...
    \(\c \o_0 t\, u(t)\ra \frac{\pi}{2}(\delta(-) + \delta(+)) + \frac{j\o}{\o_0^2-\o^2}\)\\       % write out - and +
...
\subsubsection{Uni LT Table}
    ...
    \(e^{\lambda t} u(t) \ra \frac{1}{s-\lambda}\)\\        % highlight -
...
\subsubsection{LT Prop}
    Linearity\\
    Time shift \(f(t-t_0) u(t-t_0) \ra F(s) e^{-st_0}\) Pf: \(\int_{-t_0}^{\infty}\)\\ % highlight -t0, and negative signs
    % also highlight t0 in u
    Freq shift \(f(t) e^{s_0 t} \ra F(s-s_0)\)\\
    Scaling ($a > 0$), \(f(at) \ra \frac{1}{|a|} F(\frac{s}{a})\)\\   % a > 0 for causal
    Convolution \(f_1 * f_2 \ra F_1 F_2\)\\
    \(f_1 f_2 \ra \frac{1}{2\pi j} F_1 * F_2\)

    Time diff \(\dot{f}(t) \ra sF(s) - f(0^-)\)\\     % highlight s before F and -
...
\subsubsection{Sys Anal}
    $H(s) = \frac{Y_{zs}(s)}{F(s)} = \frac{P(s)}{Q(s)}$  % highlight order, P/Q, not Q/P
    ...
    Pf. $\Rightarrow$. $|f(t)| < K$\\                       % highlight zs
    ...
    Asy $\rightarrow$ BIBO (all exp)\\                                % not the other way
    Asy m. $\rightarrow$ BIBO u. ($\int |\sin|$)                            % cross BIBO
