...
\subsubsection{Uni LT Table}
    ...
    \(e^{\lambda t} u(t) \ra \frac{1}{s-\lambda}\) ($\Re(s) > \Re(\lambda)$)\\        % highlight -
...
\subsubsection{LT Prop}
    Linearity (ROC $\cap$)\\
    Time delay \(f(t-t_0) u(t-t_0) \ra F(s) e^{-st_0}\), $t_0 > 0$ (same ROC)\\    % t0 > 0 for causal
        Pf: \(\int_{-t_0}^{\infty}\)\\ % highlight -t0, and negative signs
    ...
    Time diff \(\dot{f}(t) \ra sF(s) - f(0^-)\) ($\Re(s) > \max(\sigma_0, 0)$)\\     % highlight s before F and -
